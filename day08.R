...
decode <- function(v) {
  ...
  zero <- v[n == 6 & where(v, one) & where(v, seven) & !where(v, four)]
  six <- v[n == 6 & !where(v, zero) & !where(v, nine)]
  five <- paste0(intersect(str_split(six, "")[[1]], str_split(nine, "")[[1]]), collapse = "")

  two <- setdiff(v, c(one, three, four, five, six, seven, eight, nine, zero))
...
df <- read_delim(here("data/input/2021/08"), delim = "|", col_names =  c("input", "output")) %>%
